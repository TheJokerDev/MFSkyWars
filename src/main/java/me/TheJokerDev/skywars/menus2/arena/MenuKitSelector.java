...
public class MenuKitSelector extends Menu {
    ...
    public MenuKitSelector(Player p){
        super(p, "kitSelector", Utils.getMenuFile("Kits").getString("title"), 6);
    }
    ...
    public void onClick(InventoryClickEvent var1) {
        ...
        if (var2 != null) {
            ...
            if (Utils.compareItems(click, SkyWarsItems.getCoinsItem(var2).build())) {
                var1.setCancelled(true);
            }
            if (Utils.compareItems(click, SkyWarsItems.getBackItem().build())){
                var1.setCancelled(true);
                getPlayer().openInventory(MenuListener.getPlayerMenu(getPlayer(), "shop").getInventory());
            }
            ...
            if (var3 == null) {
                ...
                if (ConfigManager.main.getBoolean("kit_permission") && !var2.hasPermissions("skywars.kit." + var3.getName().toLowerCase())) {
                    ...
                    var2.setKit(var3);
                    var2.sendMessage(String.format(SkyWars.getMessage(MSG.PLAYER_SELECT_KIT), var3.getName()));
                }
                ...
                if (ConfigManager.shop.getBoolean("shopingame")) {
                    if (SkyWars.getPlugin().getConfig().getBoolean("kit_permission") && !var2.hasPermissions("skywars.kit." + var3.getName().toLowerCase())) {
                        ...
                        var2.setKit(var3);
                        var2.sendMessage(String.format(SkyWars.getMessage(MSG.PLAYER_PURCHASE_KIT), var3.getName()));
    ...
    public void update() {
        ...
        FileConfigurationUtil config =Utils.getMenuFile("Kits");
        if (config.get("items") != null){
        }
        if (config.get("extraItems") != null){
            for (String key : config.getSection("extraItems").getKeys(false)){
        ...
        for(int var3 = 0; var3 < var2; ++var3) {
            Kit var4 = var1[var3];
            ItemBuilder var5 = var4.getIcon().clone();
            if (var5.hasEnchantments() || var5.isGlow()){
                var5.setGlow(true);
            }
