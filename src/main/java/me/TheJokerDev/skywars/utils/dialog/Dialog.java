package me.TheJokerDev.skywars.utils.dialog;
...
public abstract class Dialog implements Listener{
    ...
    public void loadListener(){
        Bukkit.getPluginManager().registerEvents(this, SkyWars.getPlugin());
    }
    ...
    public void openDialog() {
        ...
        }.runTaskTimer(SkyWars.getPlugin(), 0, 20*2);
        loadListener();
        if (canClose()) {
            Utils.sendMessage(getPlayer(), false, Utils.getMessages().getString("General.dialogHowToClose"));
        }
    }
    ...
    private void sendTitleAndActionbar() {
        ...
        ActionBar2.sendActionBar(this.getPlayer(), this.apply(Utils.ct(this.getActionbar() != null ? this.getActionbar() : "")));
    }
    ...
    public void close() {
            ...
            ActionBar2.clearActionBar(this.getPlayer());
            this.onDialogClose();
            if (this.recall != null) {
    ...
    public void onMove(PlayerMoveEvent event) {
        if (this.canClose() && this.getPlayer().equals(event.getPlayer())) {
            ...
            if (System.currentTimeMillis() - this.lastMoved >= 5000L) {
                Utils.sendMessage(this.getPlayer(), false, Utils.getMessages().getString("General.dialogHowToClose"));
                this.lastMoved = System.currentTimeMillis();
            }
        }
    ...
    public void onClick(PlayerInteractEvent event) {
        if (this.getPlayer().equals(event.getPlayer()) && (event.getAction() == Action.LEFT_CLICK_AIR || event.getAction() == Action.LEFT_CLICK_BLOCK) && this.canClose()) {
            Utils.sendMessage(this.getPlayer(), false, Utils.getMessages().getString("General..dialogClosed"));
            this.close();
        }
