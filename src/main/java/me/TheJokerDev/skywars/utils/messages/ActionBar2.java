...
import me.TheJokerDev.skywars.utils.ReflectionUtils2;
...
public class ActionBar2 {
    ...
    static {
        ...
        if (!SPIGOT) {
            ...
            Class<?> packetPlayOutChatClass = ReflectionUtils2.getNMSClass("PacketPlayOutChat");
            Class<?> iChatBaseComponentClass = ReflectionUtils2.getNMSClass("IChatBaseComponent");

            try {
                // Game Info Message Type
                Class<?> chatMessageTypeClass = Class.forName(ReflectionUtils2.NMS + "ChatMessageType");
                ...
                Class<?> chatComponentTextClass = ReflectionUtils2.getNMSClass("ChatComponentText");
    ...
    public static void sendActionBar(Player player, String message) {
        ...
        try {
            ...
            ReflectionUtils2.sendPacket(player, packet);
     ...
     * @since 1.0.0
     */
    public static void sendActionBarWhile(JavaPlugin plugin, Player player, @Nullable String message, Callable<Boolean> callable) {
        new BukkitRunnable() {
            @Override
            public void run() {
                try {
     ...
     * @since 1.0.0
     */
    public static void sendActionBarWhile(JavaPlugin plugin, Player player, @Nullable Callable<String> message, Callable<Boolean> callable) {
        new BukkitRunnable() {
            @Override
            public void run() {
                try {
     ...
     * @since 1.0.0
     */
    public static void sendActionBar(JavaPlugin plugin, Player player, @Nullable String message, long duration) {
